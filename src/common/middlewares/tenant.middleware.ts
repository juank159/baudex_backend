...
export class TenantMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      // Estrategia 1: Obtener tenant desde subdominio (solo para dominios reales, no IPs)
      ...
      // Estrategia 2: Obtener tenant desde header personalizado (para desarrollo/testing)
      if (!tenantSlug) {
        tenantSlug = req.get('X-Tenant-Slug')?.toLowerCase() || null;
      }

      // Estrategia 3: Obtener tenant desde query parameter (solo para desarrollo)
      ...
      const isAuthEndpoint = req.path === '/api/auth/register' || req.path === '/api/auth/login';
      if (isAuthEndpoint && !tenantSlug) {
        // Permitir auth sin tenant específico - se manejará en el AuthService
        next();
        return;
      }

      // Estrategia 4: Usar organización por defecto si no se especifica
      if (!tenantSlug) {
      ...
      if (!organization) {
        throw new BadRequestException(
          `Organización no encontrada o inactiva: ${tenantSlug}`
        );
      }
      ...
      if (process.env.NODE_ENV === 'development') {
        res.set('X-Current-Tenant', tenantSlug);
      ...
      const isSystemRoute = req.path.startsWith('/health') || 
