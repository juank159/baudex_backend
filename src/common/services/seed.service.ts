...
import { Product, ProductStatus, ProductType } from '../../products/entities/product.entity';
import { ProductPrice, PriceType } from '../../products/entities/product-price.entity';
...
export class SeedService {
  ...
  async createSampleDataForOrganization(
    ...
    try {
      console.log(`🌱 Creando datos de muestra para organización: ${organizationId}`);
      ...
      // 3. Crear productos
      const products = await this.createSampleProducts(organizationId, categories);
      console.log(`✅ Creados ${products.length} productos`);

      // 4. Crear categorías de gastos
      const expenseCategories = await this.createSampleExpenseCategories(organizationId);
      console.log(`✅ Creadas ${expenseCategories.length} categorías de gastos`);

      // 5. Crear gastos
      const expenses = await this.createSampleExpenses(organizationId, expenseCategories);
      console.log(`✅ Creados ${expenses.length} gastos`);

      // 6. Crear facturas con ítems
      const invoices = await this.createSampleInvoices(organizationId, customers, products);
      console.log(`✅ Creadas ${invoices.length} facturas`);

      console.log(`🎉 Datos de muestra creados exitosamente para organización: ${organizationId}`);
    } catch (error) {
   ...
   */
  private async createSampleCategories(organizationId: string): Promise<Category[]> {
    const categoryData = [
      {
        name: 'Electrónicos',
   ...
   */
  private async createSampleCustomers(organizationId: string): Promise<Customer[]> {
    const customerData = [
      {
        firstName: 'Ana',
        lastName: 'García',
  ...
  private async createSampleProducts(
    ...
    const electronicsCategory = categories.find(c => c.name === 'Electrónicos');
    const clothingCategory = categories.find(c => c.name === 'Ropa y Accesorios');
   ...
   */
  private async createSampleExpenseCategories(organizationId: string): Promise<ExpenseCategory[]> {
    ...
    for (const data of expenseCategoryData) {
      ...
      const savedExpenseCategory = await this.expenseCategoryRepository.save(expenseCategory);
