...
export class SubscriptionExpirationService {
  ...
  async handleSubscriptionExpiration() {
    ...
    try {
      ...
      this.logger.log(`📋 Encontradas ${expiredSubscriptions.length} suscripciones expiradas`);
      ...
      this.logger.log(`✅ ${result.affected} suscripciones actualizadas a estado EXPIRED`);
      ...
      this.logger.error('❌ Error en job de expiración de suscripciones:', error);
  ...
  async handleTrialWarnings() {
    ...
    try {
      ...
      const expiringTrials = await this.subscriptionRepository
        ...
        .andWhere('subscription.isActive = :isActive', { isActive: true })
        .andWhere('subscription.endDate BETWEEN :now AND :tomorrow', { now, tomorrow })
        .getMany();

      if (expiringTrials.length > 0) {
        this.logger.warn(`⚠️ ${expiringTrials.length} trials vencen en las próximas 24 horas:`);
   ...
   */
  async forceExpireSubscriptions(): Promise<{ affected: number; subscriptions: Subscription[] }> {
    ...
    this.logger.log(`✅ Expiración manual completada: ${result.affected} suscripciones actualizadas`);
